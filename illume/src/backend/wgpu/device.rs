use {
    super::{
        convert::{from_wgpu, oom_error_from_wgpu, ToWgpu as _},
        graphics::Graphics,
    },
    crate::{
        accel::{
...
impl Device {
    ...
    pub fn create_buffer(
        ...
        if let Err(err) = result {
            ...
            return Err(oom_error_from_wgpu(err));
        }

        let address = if info.usage.contains(BufferUsage::SHADER_DEVICE_ADDRESS)
        {
            Some(Option::unwrap(from_wgpu(unsafe {
    ...
    pub fn create_graphics_pipeline(
        ...
        .result()
        .map_err(|err| oom_error_from_wgpu(err))?;
    ...
    pub fn create_image(
        ...
        match result {
            ...
            Err(err) => {
                ...
                Err(oom_error_from_wgpu(err).into())
            }
        }
    }
        ...
        T: Pod,
    {
        ...
        if let Err(err) = result {
            ...
            return Err(oom_error_from_wgpu(err).into());
        }
    ...
    pub fn create_semaphore(&self) -> Result<Semaphore, OutOfMemory> {
        let (handle, index) =
            self.create_semaphore_raw().map_err(oom_error_from_wgpu)?;
    ...
    pub fn create_shader_module(
        ...
        .map_err(|err| CreateShaderModuleError::OutOfMemoryError {
            source: oom_error_from_wgpu(err),
        })?;
    ...
    pub fn create_acceleration_structure(
        ...
        match result {
            Ok(()) => {
                ...
                let address = Option::unwrap(from_wgpu(unsafe {
